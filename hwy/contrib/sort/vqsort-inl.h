...
HWY_NOINLINE void BaseCase(D d, Traits st, T* HWY_RESTRICT keys,
  ...
  // We can avoid padding and load/store to `keys` directly after checking the
  // full array has enough. Except at the right border, it's OK to sort more
  // than the current sub-array. Even if we sort across a previous partition
  // point, we know that keys will not cross it. Use `cols` instead of full
  // vectors so we can skip some of the network stages for small sub-arrays
  // (e.g. 32..64 keys).
  if (HWY_LIKELY(keys + cols * Constants::kMaxRows <= keys_end)) {
...
bool HandleSpecialCases(D d, Traits st, T* HWY_RESTRICT keys, size_t num,
  ...
  const bool partial_128 = !detail::IsFull(d) && N < 2 && st.Is128();
